...
const TIPS = [
  'Дрифт копит буст-шкалу из трёх делений. Жми F: одно деление — синий буст, три сразу — фиолетовый УЛЬТРА!',
  'В заносе руль в поворот делает угол круче, контр-руль выравнивает. Держи контр-руль дольше секунды — занос переложится в другую сторону.',
  ...
  'Ускорения складываются: Турбо на бустере — почти вдвое быстрее, а с бустом из шкалы — ровно ×2!',
...
export class UI {
      ...
      <div class="modal controls-modal" data-modal="controls">
        <div class="panel">
          ...
          <table class="keys">
            ...
            <tr><td><kbd>Shift</kbd> / <kbd>Пробел</kbd></td><td>Прыжок и дрифт (держи в повороте — копится буст-шкала), трюк на трамплине</td></tr>
            <tr><td><kbd>F</kbd> / <kbd>X</kbd></td><td>Буст: тратит все полные деления шкалы — 1, 2 или 3 уровень</td></tr>
          ...
          <p class="muted">Геймпад: стик/крестовина — руль, RT/A — газ, LT/B — тормоз, RB/X — дрифт, Y — буст, LB — предмет. На телефоне — экранные кнопки, газ автоматический.</p>
  ...
  showResults(results, player, record = null) {
    ...
    const msgs = ['', 'Великолепно! Настоящий чемпион!', 'Почти! Ещё чуть-чуть до победы.', 'Пьедестал твой!', 'Неплохо! Попробуй дрифтовать больше.', 'Копи буст дрифтом и трать на прямых!', 'Собирай кристаллы с предметами!', 'Отстающим выпадают сильные предметы — не сдавайся!', 'Каждый чемпион с чего-то начинал!'];
